...
uint8_t Mscale = MFS_16BITS; // MFS_14BITS or MFS_16BITS, 14-bit or 16-bit
                             // magnetometer resolution
uint8_t Mmode =
    0x06; // Either 8 Hz 0x02) or 100 Hz (0x06) magnetometer data ODR
...
float GyroMeasError =
    PI * (60.0f / 180.0f); // gyroscope measurement error in rads/s (start at 60
...
int lastUpdate = 0, firstUpdate = 0,
    Now = 0; // used to calculate integration interval // used to calculate
...
public:
  ...
  void initMPU9250() {
    ...
    wait(0.1); // Delay 100 ms for PLL to get established on x-axis gyro; should
